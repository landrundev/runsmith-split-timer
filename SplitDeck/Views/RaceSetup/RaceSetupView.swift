...
struct RaceSetupView: View {
    ...
    @State private var savedRaceForStaging: Race?
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationDestination(isPresented: $navigateToStaging) {
                if let race = savedRaceForStaging {
                    let resolved = race.athleteIds.compactMap { id in
                        vm.availableAthletes.first { $0.id == id }
                    }
                    RaceStagingView(
                        race: race,
                        athletes: resolved,
        ...
        Section {
            ...
            TextField("Race Name (optional)", text: $vm.raceName)
                .onChange(of: vm.eventType) { newType in
                    ...
                    if newType.isRelay && !(newType == .relay4x400 || newType == .relay4x800) {
                        vm.splitsPerLap = 1
                    }
    ...
    private var bottomButtons: some View {
        HStack(spacing: 10) {
            ...
            // Ready to Time
            Button {
                guard let race = vm.saveRace() else { return }
                savedRaceForStaging = race
